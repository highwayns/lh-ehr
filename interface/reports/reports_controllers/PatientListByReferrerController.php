	   	...
	   	if ($_POST['form_csvexport']) {
		    ...
		    echo '"' . qescape($row['azip2']) . '"' . "\n";
		 	...
		 	echo '<tr>';
			  	...
			  	echo '</td>';
			  	echo '<td>';
					if(strlen($row['cname1']) > 0 ){ echo text($row['cname1']). " , " . text($row['aline1']). " , " . text($row['acity1']). " " .text($row['astate1']). " , " .text($row['azip1']) ;};
			  	echo '</td>';
			  	echo '<td>';
					if(strlen($row['cname2']) > 0 ){ echo text($row['cname2']). " , " . text($row['aline2']). " , " . text($row['acity2']). " " .text($row['astate2']). " , " .text($row['azip2']) ;};
			  	echo '</td>';
		 	echo '</tr>';
	   } // end not export
	   ++$totalpts;
